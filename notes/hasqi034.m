% Note: this is with transformed scores (hasqi037 is similar, but with MOS)
...
%% Training {{{
if train_a4
	myfun=@(a,x) (a(1)+a(2)*x(:,1)+a(3)*(x(:,1).^2)).*(a(4)+a(5)*x(:,2)+a(6)*x(:,3));
else
	myfun=@(a,x) (a(1)+a(2)*x(:,1)+a(3)*(x(:,1).^2)).*(1+a(4)*x(:,2)+a(5)*x(:,3));
end
